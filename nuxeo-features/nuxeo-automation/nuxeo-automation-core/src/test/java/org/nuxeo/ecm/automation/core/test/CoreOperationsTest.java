...
import org.nuxeo.ecm.automation.core.operations.document.PurgeTrash;
...
public class CoreOperationsTest {
     ...
     */
    @Test
    public void testPurgeTrash() throws IOException, OperationException {
        DocumentModel parent = session.getDocument(src.getParentRef());
        assertEquals(0, Framework.getService(TrashService.class).getDocuments(parent).size());
        ...
        ctx = new OperationContext(session);
        chain = new OperationChain("testChain");
        chain.add(PurgeTrash.ID).set("parent", parent);
        service.run(ctx, chain);
