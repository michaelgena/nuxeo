...
public class TrashServiceImpl extends DefaultComponent implements TrashService {
    ...
    public static final String TRASHED_QUERY = "SELECT * FROM Document WHERE ecm:mixinType != 'HiddenInNavigation' AND ecm:isCheckedInVersion = 0 AND ecm:isTrashed = 1 AND ecm:parentId = '%s'";
     ...
     */
    @Override
    public List<DocumentRef> getDocumentRefs(DocumentModel currentDoc) {
        CoreSession session = currentDoc.getCoreSession();
        List<DocumentRef> refs = new ArrayList<>();
        try (IterableQueryResult result = session.queryAndFetch(String.format(TRASHED_QUERY, currentDoc.getId()),
                NXQL.NXQL)) {
            for (Map<String, Serializable> map : result) {
                refs.add(new IdRef((String) map.get("ecm:uuid")));
            }
        }
        return refs;
    }
